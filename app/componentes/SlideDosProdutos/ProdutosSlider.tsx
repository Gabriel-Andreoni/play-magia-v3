...
export function ProdutosSlider({ fotos }: ProdutosSliderProps) {
  const [playgroundSelecionado, setPlaygroundSelecionado] =
    useState<PlaygroundInfo>();

  useEffect(() => {
    const playgroundInfo: PlaygroundInfo = JSON.parse(
      localStorage.getItem("playgroundInfo") || "{}",
    );
